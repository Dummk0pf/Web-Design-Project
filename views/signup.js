...
const wrongInputCloseButton = document.getElementById(
  "wrong-credentials-close-button"
);
...
const checkDetails = function () {
  ...
  if (email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)?.input === email) {
    ...
    if (
      ...
      throwMsg(
        "Successfully Signed Up 🤩🤩 Sign In to Continue (redirecting to Sign in page)"
      );
      setTimeout(() => {
      ...
      throwMsg("Min length must be 8 (include a number and spl character)");
    }
  } else {
